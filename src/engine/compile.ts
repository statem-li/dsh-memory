...
import { isInjectionEligible, injectionRank, shouldPromote } from './scoring.ts'
import { localDate, projectHashOf, type MemoryStore } from './store.ts'
...
export function buildInjectionText(
  ...
  const sections: InjectionSections = { identity: '', memory: '', pinned: '', facts: '' }
  const pinned = entries.filter(entry => entry.pinned)
  const rest = entries.filter(entry => !entry.pinned)
  // 预算内逐条累积：pinned 无条件先放，其余按 importance 降序。
  let used = 0
  const consume = (section: keyof InjectionSections, text: string): void => {
    if (text === '') return
  ...
  for (const entry of rest) {
    if (entry.scope === 'global') {
      if (isIdentityEntry(entry)) {
        ...
        if (sections.facts === '') consume('facts', `- ${entry.content}`)
      }
    } else {
      if (sections.memory === '') consume('memory', `- ${entry.content}`)
    }
  }
  const text = [
    sections.identity,
