#!/bin/bash
# Build dsh-memory: link build deps from the dsh checkout, then run tsdown
# (host half → lib/index.js, client half → lib/client.js).
...
if [ -n "$CLSX_DIR" ]; then
  node -e "
    const fs = require('fs');
    const path = require('path');
    const link = path.resolve('node_modules/clsx');
    const target = path.resolve(process.argv[1] + '/node_modules/clsx');
    fs.rmSync(link, { recursive: true, force: true });
    fs.mkdirSync(path.dirname(link), { recursive: true });
    fs.symlinkSync(target, link, process.platform === 'win32' ? 'junction' : 'dir');
  " "$CLSX_DIR"
...
"$CHECKOUT/node_modules/.bin/tsdown" 2>/dev/null || pnpm exec tsdown
